...
/**
 * Performs a logical XNOR operation on two boolean operands, returning a type that represents the result of the operation.
 *
 * # Resulting operand
 ...
 * | `false` | `false` | `true` |
 * > [!NOTE]
 * > The {@linkcode UseInversion} capability can be used to flip the result of the XNOR operation, resulting in `false` if the resulting operand is `true`, and `true` otherwise.
 * # Capabilities
 * - {@link UseCondition}
 * - {@link UseInversion}
 * - {@link UseStream}
...
type Result<OperandA extends boolean, OperandB extends boolean> =
  OperandA extends true ? OperandB extends true ? true : false
