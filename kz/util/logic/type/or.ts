...
/**
 * Performs a logical OR operation on two boolean operands, returning a type that represents the result of the operation.
 *
 * # Resulting operand
 ...
 * | `false` | `false` | `false` |
 * > [!NOTE]
 * > The {@linkcode UseInversion} capability can be used to flip the result of the OR operation, resulting in `false` if the resulting operand is `true`, and `true` otherwise.
 * # Capabilities
 * - {@link UseCondition}
 * - {@link UseInversion}
 * - {@link UseStream}
...
type Result<OperandA extends boolean, OperandB extends boolean> =
  OperandA extends true ? true : OperandB extends true ? true : false;
