// Copyright 2020 - present integereleven. All rights reserved. MIT license.

// deno-lint-ignore-file no-boolean-literal-for-arguments
import { describe, it } from '@std/testing/bdd';
import { assertType, type IsExact } from '@std/testing/types';
import type { AsDistributed, AsLoose } from '@kz/util/capability';

import type { Any } from '../../any.ts';

import type { CheckSpecial } from './check_special.ts';

type AnyType = Any<'i11n#testing'>;

/**
 * Test that the `CheckSpecial` type correctly determines whether a given type is `special` based on the specified settings and capabilities, and checks for type compatibility accordingly.
 *
 * This tests the matrix of `UseStrict` and `UseDistributed` capabilities.
 *
 * - Strict unified - `AsStrict & AsUnified` (default)
 * - Strict distributed - `AsStrict & AsDistributed`
 * - Loose unified - `AsLoose & AsUnified`
 * - Loose distributed - `AsLoose & AsDistributed`
 */
describe('CheckSpecial', () => {
  describe('unknown', () => {
    type TypeToCheck = unknown;

    describe('AsStrict (default)', () => {
      describe('AsUnified (default)', () => {
        it('should return true for any type', () => {
          type Actual = CheckSpecial<TypeToCheck, AnyType>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for never type', () => {
          type Actual = CheckSpecial<TypeToCheck, never>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for void type', () => {
          type Actual = CheckSpecial<TypeToCheck, void>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for unknown type', () => {
          type Actual = CheckSpecial<TypeToCheck, unknown>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for string type', () => {
          type Actual = CheckSpecial<TypeToCheck, string>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });

      describe('AsDistributed', () => {
        it('should return true for any type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            TypeToCheck,
            AsDistributed
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for never type', () => {
          type Actual = CheckSpecial<TypeToCheck, never, AsDistributed>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for void type', () => {
          type Actual = CheckSpecial<TypeToCheck, void, AsDistributed>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for unknown type', () => {
          type Actual = CheckSpecial<TypeToCheck, unknown, AsDistributed>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for string type', () => {
          type Actual = CheckSpecial<TypeToCheck, string, AsDistributed>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });
    });

    describe('AsLoose', () => {
      describe('AsUnified (default)', () => {
        it('should return true for any type', () => {
          type Actual = CheckSpecial<TypeToCheck, TypeToCheck, AsLoose>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for never type', () => {
          type Actual = CheckSpecial<TypeToCheck, never, AsLoose>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for void type', () => {
          type Actual = CheckSpecial<TypeToCheck, void, AsLoose>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for unknown type', () => {
          type Actual = CheckSpecial<TypeToCheck, unknown, AsLoose>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for string type', () => {
          type Actual = CheckSpecial<TypeToCheck, string, AsLoose>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });

      describe('AsDistributed', () => {
        it('should return true for any type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            TypeToCheck,
            AsDistributed & AsLoose
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for never type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            never,
            AsDistributed & AsLoose
          >;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for void type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            void,
            AsDistributed & AsLoose
          >;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for unknown type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            unknown,
            AsDistributed & AsLoose
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for string type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            string,
            AsDistributed & AsLoose
          >;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });
    });
  });

  describe('never', () => {
    type TypeToCheck = never;

    describe('AsStrict (default)', () => {
      describe('AsUnified (default)', () => {
        it('should return false for any type', () => {
          type Actual = CheckSpecial<TypeToCheck, AnyType>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for never type', () => {
          type Actual = CheckSpecial<TypeToCheck, never>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for void type', () => {
          type Actual = CheckSpecial<TypeToCheck, void>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for unknown type', () => {
          type Actual = CheckSpecial<TypeToCheck, unknown>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for string type', () => {
          type Actual = CheckSpecial<TypeToCheck, string>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });

      describe('AsDistributed', () => {
        it('should return true for any type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            TypeToCheck,
            AsDistributed
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for never type', () => {
          type Actual = CheckSpecial<TypeToCheck, never, AsDistributed>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for void type', () => {
          type Actual = CheckSpecial<TypeToCheck, void, AsDistributed>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for unknown type', () => {
          type Actual = CheckSpecial<TypeToCheck, unknown, AsDistributed>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for string type', () => {
          type Actual = CheckSpecial<TypeToCheck, string, AsDistributed>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });
    });

    describe('AsLoose', () => {
      describe('AsUnified (default)', () => {
        it('should return true for any type', () => {
          type Actual = CheckSpecial<TypeToCheck, TypeToCheck, AsLoose>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for never type', () => {
          type Actual = CheckSpecial<TypeToCheck, never, AsLoose>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for void type', () => {
          type Actual = CheckSpecial<TypeToCheck, void, AsLoose>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for unknown type', () => {
          type Actual = CheckSpecial<TypeToCheck, unknown, AsLoose>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for string type', () => {
          type Actual = CheckSpecial<TypeToCheck, string, AsLoose>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });

      describe('AsDistributed', () => {
        it('should return true for any type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            TypeToCheck,
            AsDistributed & AsLoose
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for never type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            never,
            AsDistributed & AsLoose
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for void type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            void,
            AsDistributed & AsLoose
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for unknown type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            unknown,
            AsDistributed & AsLoose
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for string type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            string,
            AsDistributed & AsLoose
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });
    });
  });

  describe('void', () => {
    type TypeToCheck = void;

    describe('AsStrict (default)', () => {
      describe('AsUnified (default)', () => {
        it('should return false for any type', () => {
          type Actual = CheckSpecial<TypeToCheck, AnyType>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for never type', () => {
          type Actual = CheckSpecial<TypeToCheck, never>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for void type', () => {
          type Actual = CheckSpecial<TypeToCheck, void>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for unknown type', () => {
          type Actual = CheckSpecial<TypeToCheck, unknown>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for string type', () => {
          type Actual = CheckSpecial<TypeToCheck, string>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });

      describe('AsDistributed', () => {
        it('should return true for any type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            TypeToCheck,
            AsDistributed
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for never type', () => {
          type Actual = CheckSpecial<TypeToCheck, never, AsDistributed>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for void type', () => {
          type Actual = CheckSpecial<TypeToCheck, void, AsDistributed>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for unknown type', () => {
          type Actual = CheckSpecial<TypeToCheck, unknown, AsDistributed>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for string type', () => {
          type Actual = CheckSpecial<TypeToCheck, string, AsDistributed>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });
    });

    describe('AsLoose', () => {
      describe('AsUnified (default)', () => {
        it('should return true for any type', () => {
          type Actual = CheckSpecial<TypeToCheck, TypeToCheck, AsLoose>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for never type', () => {
          type Actual = CheckSpecial<TypeToCheck, never, AsLoose>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for void type', () => {
          type Actual = CheckSpecial<TypeToCheck, void, AsLoose>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for unknown type', () => {
          type Actual = CheckSpecial<TypeToCheck, unknown, AsLoose>;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for string type', () => {
          type Actual = CheckSpecial<TypeToCheck, string, AsLoose>;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });

      describe('AsDistributed', () => {
        it('should return true for any type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            TypeToCheck,
            AsDistributed & AsLoose
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for never type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            never,
            AsDistributed & AsLoose
          >;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for void type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            void,
            AsDistributed & AsLoose
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return true for unknown type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            unknown,
            AsDistributed & AsLoose
          >;
          type Expected = true;

          assertType<IsExact<Actual, Expected>>(true);
        });

        it('should return false for string type', () => {
          type Actual = CheckSpecial<
            TypeToCheck,
            string,
            AsDistributed & AsLoose
          >;
          type Expected = false;

          assertType<IsExact<Actual, Expected>>(true);
        });
      });
    });
  });
});
